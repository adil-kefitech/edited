...
class ExamConfiguration(ListAPIView):
    ...
    def post(self,request):
        try:
            ...
            #     exam.save()
                # examid=existance_check.id


            # if exam.is_valid(raise_exception=True):
            #     # serializer.save() returns the newly created object instance
            #     saved_exam = exam.save() 
            #     examid = saved_exam.id
            # if exam.is_valid(raise_exception=True):
            #     saved_exam = exam.save()
                
            #     # Safely check if the serializer returned a list or a single object
            #     if isinstance(saved_exam, list):
            #         examid = saved_exam[0].id  # Grab the first object in the list
            #     else:
            # 
            #         examid = saved_exam.id     # Grab the object directly
            
            if exam.is_valid(raise_exception=True):
                saved_exam = exam.save()
                
                # Because the old serializer returns a list, we MUST grab index [0]
                if isinstance(saved_exam, list):
                ...
                # if examnotobj==None:
                #     notificationobj=ExaminationNotification(created_on=created_on,created_by_id=user,date=notif_date,status_id=ACTIVE_STATE,attendance_upload_end_date=attendence_lastdate,exam_prgm_sem_id=exmprosemid)
    ...
    def get(self, request): 
        try:
            ...
            return format_response(
                ...
                data={
                    "exam_list": [], # INSTANT PAGE LOAD!
                    "prg_typ": programme_type,
                    "prg_grp": programme_group,
                    "status": exam_status,
                    "admnyear": admnyear,
                    "cascade_json": json.dumps(cascade_data) # Send map to frontend
                },
...
class ExamSingleGet(ListAPIView):
    def get(self, request, pk):
        try:
            ...
            # Each entry for this exam can independently mark an admission
            # year as Regular-chance and / or Improvement.
            # -------------------------------------------------------
            # exam_adm_years = ExaminationAdmissionYear.objects.filter(
            #     exam_id=pk
            # ).select_related('admission_year').order_by('admission_year__admission_year')
            # -------------------------------------------------------
            # Categorise admission years using the Regular / improvement
            # flags stored on ExaminationAdmissionYear (the through table).
            # -------------------------------------------------------
            ...
            for eay in exam_adm_years:
                ...
                # all_years — every linked admission year (de-duplicated)
            ...
            return format_response(
                ...
                data={
                    ...
                    # These three are passed separately so the template does NOT
                    # have to re-parse admissionYr and guess the flags.
...
class ExamEditlist(ListAPIView):
    def get(self,request,pk):
        try:
            ...
            # ── Existing ExaminationAdmissionYear entries for this exam with flags ──
            ...
            # # Build categorised ID lists from the through-table flags
            # existing_all_ids     = []  # Every linked year
            # existing_regular_ids = []  # Regular=True
            # existing_impr_ids    = []  # improvement=True
            # existing_supp_ids    = []  # Both False → supplementary

            # for eay in exam_adm_year_qs:
            #     yr_id = eay.admission_year.id
            #     if yr_id not in existing_all_ids:
            #         existing_all_ids.append(yr_id)
            #     if eay.Regular and yr_id not in existing_regular_ids:
            #         existing_regular_ids.append(yr_id)
            #     if eay.improvement and yr_id not in existing_impr_ids:
            #         existing_impr_ids.append(yr_id)
            #     if not eay.Regular and not eay.improvement and yr_id not in existing_supp_ids:
            #         existing_supp_ids.append(yr_id)

            # # Single IDs for the three single-select dropdowns
            ...
            # # We need to find the specific year ID that the user ACTUALLY intended for Supplementary.
            # # To do this safely amidst ghost records, we look at what was submitted most recently.
            # for eay in exam_adm_year_qs:
            #     yr_id = eay.admission_year.id
            #     if yr_id not in existing_all_ids:
            #         existing_all_ids.append(yr_id)
            ...
            #     if eay.Regular and yr_id not in existing_regular_ids:
            #         existing_regular_ids.append(yr_id)
            #     elif eay.improvement and yr_id not in existing_impr_ids:
            #         existing_impr_ids.append(yr_id)
            #     elif not eay.Regular and not eay.improvement:
            #         # It's supplementary. But we might have ghosts. 
            #         # We will collect all of them for now.
            #         if yr_id not in existing_supp_ids:
            #             existing_supp_ids.append(yr_id)

            # # --- THE GHOST BUSTER LOGIC ---
            # # If we have multiple "supplementary" years (due to failed deletions),
            # # we sort the query set by 'updated_on' to find the one that was intentionally
            # # saved last. The ghosts will be older.
            # # valid_supp_id = None
            # # if existing_supp_ids:
            # #     # Fetch the most recently updated supplementary record
            # #     latest_supp_record = ExaminationAdmissionYear.objects.filter(
            # #         exam__in=examobj,
            # #         Regular=False,
            # #         improvement=False
            # #     ).order_by('-updated_on').first()

            # #     if latest_supp_record:
            # #          valid_supp_id = latest_supp_record.admission_year.id

            # # existing_regular_id = existing_regular_ids[0] if existing_regular_ids else None
            # # existing_impr_id    = existing_impr_ids[0]    if existing_impr_ids    else None
            # # existing_supp_id    = valid_supp_id # Use the verified ID
            ...
            # #         # Single-select pre-selection values
            ...
            # valid_supp_id = None
            # if existing_supp_ids:
            #     # Fetch the most recently updated ACTIVE supplementary record
            #     latest_supp_record = ExaminationAdmissionYear.objects.filter(
            #         exam__in=examobj,
            #         Regular=False,
            #         improvement=False,
            #         status_id=ACTIVE_STATE  # <-- THIS IS THE MAGIC FIX
            #     ).order_by('-updated_on').first()
            ...
            # #  THE FIX: Clear out the ghost records by strictly rebuilding the master list
            # # using ONLY the verified active IDs we just calculated above.
            # existing_all_ids = []
            # if existing_regular_id:
            #     existing_all_ids.append(existing_regular_id)
            # if existing_impr_id:
            #     existing_all_ids.append(existing_impr_id)
            # if existing_supp_id:
            #     existing_all_ids.append(existing_supp_id)
            ...
            #         'not_list': serilizer_data.data,
            #         'all_adm_years':       all_adm_years,
            #         'existing_all_ids':    existing_all_ids, # Now perfectly clean!
            #         'existing_regular_id': existing_regular_id,
            #         'existing_impr_id':    existing_impr_id,
            #         'existing_supp_id':    existing_supp_id,
            #     },
            ...
            # existing_all_ids = []
            # if existing_regular_id: existing_all_ids.append(existing_regular_id)
            # if existing_impr_id:    existing_all_ids.append(existing_impr_id)
            # if existing_supp_id:    existing_all_ids.append(existing_supp_id)
    ...
    def post(self, request):
        ...
        try:
            ...
            for date in exam_date:
                ...
                # ── Bug 1 fix: safe date parse — handles both YYYY-MM-DD and YYYY-M-D ──
                # try:
                #     startdate = datetime.strptime(s.strip(), "%Y-%m-%d").date()
                #     enddate   = datetime.strptime(e.strip(), "%Y-%m-%d").date()
                # except (ValueError, AttributeError) as date_err:
                #     logger.error(f"Invalid purpose date for purposeId {purpose_id}: start='{s}' end='{e}' — {date_err}")
                #     return format_response(False, f"Invalid date value for purpose '{purpose_id}': '{s}' or '{e}'",
            ...
            # ── Bug 6 fix: notification may not exist yet ──
            ...
            # ── Bug 1 fix: dates are already YYYY-MM-DD from the fixed frontend ──
            notif_date_str = request.data.get('notif_date', '').strip()
            uplod_date_str = request.data.get('attendance_upload_end_date', '').strip()

            # try:
            #     notification.date = datetime.strptime(notif_date_str, "%Y-%m-%d").date()
            # except (ValueError, AttributeError):
            #     return format_response(False, f"Invalid notification date: '{notif_date_str}'",
            #                            {}, BAD_GATEWAY_ERROR_CODE, status_code=status.HTTP_400_BAD_REQUEST)

            # try:
            #     notification.attendance_upload_end_date = datetime.strptime(uplod_date_str, "%Y-%m-%d").date()
            # except (ValueError, AttributeError):
            #     return format_response(False, f"Invalid attendance upload date: '{uplod_date_str}'",
            #                            {}, BAD_GATEWAY_ERROR_CODE, status_code=status.HTTP_400_BAD_REQUEST)

            # Create a quick helper function to safely test multiple date formats
            # def safe_parse_date(date_string):
            #     if not date_string:
            #         raise ValueError("Date string is empty")

            #     # It will try YYYY-MM-DD first, then fallback to DD/MM/YYYY, then MM/DD/YYYY
            ...
            #     # ✨ Added "%d-%m-%Y" to catch dates like 11-07-2025
            #     for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d-%m-%Y"):
            #         try:
            #             return datetime.strptime(date_string, fmt).date()
            #         except ValueError:
            ...
            # ── Bug 4 fix: frontend sends 'myFile', not 'my_file' ──
            # uploaded_file = request.data.get('myFile') or request.data.get('my_file')
            # if uploaded_file and hasattr(uploaded_file, 'read'):
            #     notification.notification_url = base64.b64encode(uploaded_file.read())

            uploaded_file = request.data.get('myFile') or request.data.get('my_file')
            if uploaded_file and hasattr(uploaded_file, 'read'):
                notification.notification_url = uploaded_file.read()  # store raw bytes, not base64

            notification.save()

            # ── Update ExaminationAdmissionYear (Regular + Improvement + Supplementary flags) ──
            # Field names match exam-add (exam_new.html) exactly:
            #   admissionYearList          → all considered year IDs (auto-synced multiselect)
            #   admissionyear_regular_list → Regular-chance year ID (single select)
            #   admissionimprovement       → Improvement year ID (single select, only sent when YES)
            #   supplementaryYearList      → Supplementary year ID (single select)


            # adm_year_list      = request.data.getlist('admissionYearList')
            # regular_list       = request.data.getlist('admissionyear_regular_list')
            # improvement_list   = request.data.getlist('admissionimprovement')
            # supplementary_list = request.data.getlist('supplementaryYearList')

            # # Union of all lists — any year in any category is "considered"
            # all_year_ids = list(dict.fromkeys(
            #     adm_year_list + regular_list + improvement_list + supplementary_list
            # ))
            ...
            # 🔥 THE TRAP: Print the raw truth to the terminal!
            print("==== DIAGNOSTIC TRAP ====")
            print("1. RAW PAYLOAD:", request.data)
            print("2. SUPPLEMENTARY LIST:", supplementary_list)
            print("3. FINAL IDs TO SAVE:", all_year_ids)
            print("=========================")

            # if all_year_ids:
            #     ExaminationAdmissionYear.objects.filter(exam=exam).delete()
            #     for yr_id in all_year_ids:
            #         is_regular     = yr_id in regular_list
            #         is_improvement = yr_id in improvement_list
            #         # Supplementary: not regular and not improvement → both flags False
            #         # If improvement is also selected for the same year → improvement wins
            ...
            # ── Fix for Django-River ProtectedError: Sync instead of Delete ──
            # if all_year_ids:
            #     # 1. Fetch existing records into a dictionary keyed by year ID
            ...
            #     for yr_id in all_year_ids:
            #         yr_str = str(yr_id)
            #         is_regular     = yr_str in regular_list
            #         is_improvement = yr_str in improvement_list

            #         if yr_str in eay_dict:
            #             # Update existing record instead of recreating it
            #             eay_obj = eay_dict[yr_str]
            #             eay_obj.Regular = is_regular
            #             eay_obj.improvement = is_improvement
            #             eay_obj.updated_by = updated_by
            #             eay_obj.updated_on = updated_on
            #             eay_obj.save()
            ...
            #     # 2. Handle any years that were UNCHECKED by the user
            #     from django.db.models.deletion import ProtectedError
            #     for leftover_eay in eay_dict.values():
            #         try:
            #             leftover_eay.delete()
            #         except ProtectedError:
            #             # If Django-River blocks deletion, safely disable the flags
            #             leftover_eay.Regular = False
            #             leftover_eay.improvement = False
            #             leftover_eay.updated_by = updated_by
            #             leftover_eay.updated_on = updated_on
            #             leftover_eay.save()
            ...
            if all_year_ids:
                ...
                #         eay_obj.Regular = is_regular
                #         eay_obj.improvement = is_improvement
                #         eay_obj.status_id = ACTIVE_STATE  # Reactivate in case it was previously soft-deleted
                #         eay_obj.updated_by = updated_by
                #         eay_obj.updated_on = updated_on
                #         eay_obj.save()
                #         del eay_dict[yr_str]
                ...
                # #         leftover_eay.delete()
                # #     except ProtectedError:
                # #         # Soft-delete the record so it doesn't show up as Supplementary
                # #         leftover_eay.status_id = DELETE_STATE
                # #         leftover_eay.Regular = False
                # #         leftover_eay.improvement = False
                # #         leftover_eay.updated_by = updated_by
                # #         leftover_eay.updated_on = updated_on
                ...
                #         leftover_eay.delete()
                #     except ProtectedError:
                #         # If Django-River blocks deletion, zero out the flags to make it a "ghost" supplementary
                #         leftover_eay.Regular = False
                #         leftover_eay.improvement = False
                #         leftover_eay.updated_by = updated_by
                #         leftover_eay.updated_on = updated_on # Crucial: Update the timestamp
                #         leftover_eay.save()
                ...
                for yr_id in all_year_ids:
                    ...
                    if existing_records.exists():
                        ...
                        # If the old bug created duplicates, force the clones to DELETE (5)
